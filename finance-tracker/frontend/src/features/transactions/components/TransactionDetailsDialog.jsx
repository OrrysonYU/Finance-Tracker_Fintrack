...
export function TransactionDetailsDialog({ accounts, onClose, onEdit, open, transaction }) {
  ...
  return <Dialog open={open} onClose={onClose} className="ui-dialog"><DialogBackdrop className="ui-dialog__backdrop" /><div className="ui-dialog__positioner"><DialogPanel as={motion.div} initial={reduceMotion ? false : { opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="transaction-details">
    <div className="transaction-details__header"><span className={`transaction-details__icon transaction-details__icon--${transaction.is_credit ? "income" : "expense"}`}><DirectionIcon size={21} aria-hidden="true" /></span><div><p>Transaction details</p><DialogTitle>{transaction.description || "Untitled transaction"}</DialogTitle><DialogDescription className="sr-only">Review the amount, account, category, date, and time for this transaction.</DialogDescription></div><button type="button" className="finance-icon-button" onClick={onClose} aria-label="Close transaction details"><X size={18} aria-hidden="true" /></button></div>
