...
export function TransactionWorkspaceFilters({ accounts, categories, filters, onChange, onReset, search, onSearchChange }) {
  ...
  return <section className="transaction-filters" aria-label="Transaction filters">
    <div className="transaction-filters__topline">
      ...
      <Button variant="secondary" className="transaction-filters__advanced-button" onClick={() => setExpanded((value) => !value)} aria-expanded={expanded} aria-controls="transaction-advanced-filters"><SlidersHorizontal size={16} aria-hidden="true" />Filters{activeCount > 0 && <span className="transaction-filters__count">{activeCount}</span>}<ChevronDown size={15} className={expanded ? "transaction-filters__chevron--open" : ""} aria-hidden="true" /></Button>
    </div>
    <AnimatePresence initial={false}>{expanded && <motion.div id="transaction-advanced-filters" initial={reduceMotion ? false : { opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={reduceMotion ? { opacity: 0 } : { opacity: 0, height: 0 }} className="transaction-filters__advanced">
