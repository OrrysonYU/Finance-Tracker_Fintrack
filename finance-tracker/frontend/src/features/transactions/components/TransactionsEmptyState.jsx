...
export function TransactionsEmptyState({ hasAccounts, onCreate }) {
  return (
    <section className="glass overflow-hidden rounded-3xl border border-dashed border-blue-400/30 p-8 text-center">
      ...
      <h2 className="mt-5 text-xl font-semibold">No transactions yet</h2>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-[var(--color-muted)]">
        {hasAccounts
          ? "Add your first income or expense to start building your ledger history."
          : "Create an account first, then transactions can be linked to it safely."}
      </p>
      {hasAccounts && (
        <button
          type="button"
