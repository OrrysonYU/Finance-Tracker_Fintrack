...
export default function LandingPage() {
  ...
  return <div className="landing-page">
    ...
    <header className="landing-nav"><div className="landing-container landing-nav__inner"><Link to="/" className="landing-brand" aria-label="Fintrack home" onClick={closeMenu}><FintrackLogo size={30} decorative /><span>fintrack</span></Link><div className="landing-nav__actions"><ThemeToggle /><button ref={menuButtonRef} type="button" className="landing-menu" aria-label={menuOpen ? "Close navigation" : "Open navigation"} aria-controls="landing-primary-navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}>{menuOpen ? <X size={21} /> : <Menu size={21} />}</button></div><nav id="landing-primary-navigation" className={`landing-nav__links ${menuOpen ? "is-open" : ""}`} aria-label="Primary navigation"><a href="#product" onClick={closeMenu}>Product</a><a href="#intelligence" onClick={closeMenu}>Intelligence</a><a href="#trust" onClick={closeMenu}>Trust</a><Link to="/login" onClick={closeMenu}>Sign in</Link><Link className="landing-button landing-button--dark landing-button--nav" to="/register" onClick={closeMenu}>Create account <ArrowUpRight size={15} /></Link></nav></div></header>
