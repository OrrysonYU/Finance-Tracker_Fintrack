    ...
    <div className="landing-preview" aria-label="Illustration of the Fintrack dashboard" role="img">
      ...
      <div className="landing-preview__body">
        ...
        <div className="landing-preview__content">
          <div className="landing-preview__topline"><div><small>Financial command center</small><h3>Your overview</h3></div><span className="landing-preview__date">This month⌄</span></div>
...
export default function LandingPage() {
  ...
  return <main className="landing-page">
    ...
    <header className="landing-nav"><div className="landing-container landing-nav__inner"><Link to="/" className="landing-brand" aria-label="Fintrack home" onClick={closeMenu}><FintrackLogo size={30} /><span>fintrack</span></Link><nav id="landing-primary-navigation" className={`landing-nav__links ${menuOpen ? "is-open" : ""}`} aria-label="Primary navigation"><a href="#product" onClick={closeMenu}>Product</a><a href="#intelligence" onClick={closeMenu}>Intelligence</a><a href="#trust" onClick={closeMenu}>Trust</a><Link to="/login" onClick={closeMenu}>Sign in</Link><Link className="landing-button landing-button--dark landing-button--nav" to="/register" onClick={closeMenu}>Create account <ArrowUpRight size={15} /></Link></nav><button type="button" className="landing-menu" aria-label={menuOpen ? "Close navigation" : "Open navigation"} aria-controls="landing-primary-navigation" aria-expanded={menuOpen} onClick={() => setMenuOpen((open) => !open)}>{menuOpen ? <X size={21} /> : <Menu size={21} />}</button></div></header>
    <div id="main-content">
    ...
    <footer className="landing-footer"><div className="landing-container landing-footer__top"><div><Link to="/" className="landing-brand"><FintrackLogo size={27} variant="inverse" /><span>fintrack</span></Link><p>A calmer, clearer way to understand your finances.</p></div><div className="landing-footer__links"><div><b>Product</b><a href="#product">Overview</a><a href="#intelligence">AI Insights</a><a href="#accounts">Accounts</a><a href="#reports">Reports</a></div><div><b>Access</b><Link to="/login">Sign in</Link><Link to="/register">Create account</Link></div><div><b>Principles</b><a href="#trust">Security & privacy</a><a href="#intelligence">AI boundaries</a></div></div></div><div className="landing-container landing-footer__bottom"><span>© {new Date().getFullYear()} Fintrack</span><span>Made for clearer financial decisions.</span></div></footer>
  </main>;
}
